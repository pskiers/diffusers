...
def _rollout_trajectory(
    ...
    Returns: (pred_skip_list, actual_cost_list) — lists of (B,) tensors, one
    per eligible denoising step. The generated image itself is discarded:
    this script only cares about the per-step comparison, not the final
    output (see ablate_trm_loop_budget.py for the end-to-end check).
    """
    ...
    pred_skip_list, actual_cost_list = [], []
    ...
    return pred_skip_list, actual_cost_list
...
def main(cfg: DictConfig):
    ...
    all_pred_skip, all_actual_cost = [], []
    for cb in tqdm(cached_batches, desc="Rollout"):
        pred_skip_list, actual_cost_list = _rollout_trajectory(
    ...
    return {"discrimination": discrimination, "threshold_sweep": threshold_sweep}
