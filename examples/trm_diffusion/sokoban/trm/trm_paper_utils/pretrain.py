...
def evaluate(
    ...
    with torch.inference_mode():
        ...
        # SOKOBAN metrics
        sokoban_eval_metrics = {}
        if rank == 0 and "logits" in save_preds:
            print("\nComputing domain metrics with SokobanEvaluator...")
            from ...sokoban_utils import SokobanEvaluator

            # Ekstrakcja predykcji z logitów (zmiana na 2D)
            logits_tensor = save_preds["logits"] # [N, 144, 8]
            preds_1d = logits_tensor.argmax(dim=-1).cpu().numpy()
            boards_2d = preds_1d.reshape(-1, 12, 12)

            rng = np.random.default_rng(config.seed + train_state.step)
            subset_size = min(100, len(boards_2d))
            subset_idx = rng.choice(len(boards_2d), subset_size, replace=False)
            eval_subset = boards_2d[subset_idx]

            soko_eval = SokobanEvaluator(num_boxes=4)
            soko_metrics = soko_eval.generate_metrics(generated_boards=eval_subset)
        ...
        if rank == 0:
            ...
            if sokoban_eval_metrics:
                if reduced_metrics is None:
                    reduced_metrics = {}
                reduced_metrics.update(sokoban_eval_metrics)
