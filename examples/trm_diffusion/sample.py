...
from sokoban.evaluate_sokoban import boards_from_bit_images, compute_sokoban_metrics
...
def main(args: DictConfig):
    ...
    for b_idx in tqdm(range(num_batches), disable=not accelerator.is_local_main_process):
        ...
        if prompt_iterator is not None:
            ...
            try:
                cond_batch = next(prompt_iterator)
            ...
            if cond_batch is not None and cond_batch.get("conditions") is not None:
                ...
                num_conds = cond_imgs.shape[0]

                raw_labels = None
                if is_class_conditional and cond_batch.get("class_labels") is not None:
                    raw_labels = cond_batch["class_labels"]

                if num_conds >= current_bsz:
                    prompt = cond_imgs[:current_bsz].to(accelerator.device, dtype=torch.float32)
                    if raw_labels is not None:
                        class_labels = raw_labels[:current_bsz].to(accelerator.device)
                else:
                    repeats = math.ceil(current_bsz / num_conds)
                    prompt = cond_imgs.repeat(repeats, 1, 1, 1)[:current_bsz].to(accelerator.device, dtype=torch.float32)
                    if raw_labels is not None:
                        class_labels = raw_labels.repeat(repeats)[:current_bsz].to(accelerator.device)
        ...
        if is_sokoban:
            ...
            all_gen_boards_list.append(gen_boards)

            if is_concat_cond and prompt is not None:
                cond_boards = boards_from_bit_images((prompt.cpu().float() / 2 + 0.5).clamp(0, 1))
                all_cond_boards_list.append(cond_boards)


    if is_sokoban and all_gen_boards_list:
        all_boards = np.concatenate(all_gen_boards_list, axis=0)
        cond_boards_final = np.concatenate(all_cond_boards_list, axis=0) if all_cond_boards_list else None
