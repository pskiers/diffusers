#!/bin/bash -l
#SBATCH --job-name=amaze_maze_dit_v3
...
# Standalone (no-TRM) diffusion baseline for MAZE — "V3" data regime:
...
#   sbatch slurm_scripts/train_maze_dit_v3.sh [WANDB_PROJECT] [RUN_NAME]
...
RUN_NAME="${RUN_NAME:-${2:-maze_dit_v3${SLURM_JOB_ID:+_${SLURM_JOB_ID}}}}"
...
srun python train_trm.py experiment=amaze_dit_maze_v3 \
...
echo "Maze DiT V3 complete → runs/${RUN_NAME}/checkpoint_final.pt"
