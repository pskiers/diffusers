#!/bin/bash -l
#SBATCH --job-name=amaze_queens_dit
...
# Standalone (no-TRM) diffusion baseline for QUEENS.
...
#   sbatch slurm_scripts/train_queens_dit.sh [WANDB_PROJECT] [RUN_NAME]
...
RUN_NAME="${RUN_NAME:-${2:-queens_dit${SLURM_JOB_ID:+_${SLURM_JOB_ID}}}}"
...
srun python train_trm.py experiment=amaze_dit_queens \
...
echo "Queens DiT complete → runs/${RUN_NAME}/checkpoint_final.pt"
