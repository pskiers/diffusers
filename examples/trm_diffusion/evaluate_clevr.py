...
# Joint Prompts force the model to resolve color and material together
...
# --- 3D CALIBRATION ---
def calibrate_camera_and_size(scenes, num_samples=150):
    ...
    for scene in scenes[:num_samples]:
        for obj in scene['objects']:
            ...
            if obj['size'] == 'small': small_widths_3d.append(w_3d)
            else: large_widths_3d.append(w_3d)
        for r_type in ['left', 'front']:
            for i, targets in enumerate(scene['relationships'][r_type]):
                for j in targets:
                    diff = np.array(scene['objects'][j]['3d_coords'][:2]) - np.array(scene['objects'][i]['3d_coords'][:2])
                    if r_type == 'left': left_diffs.append(diff)
                    else: front_diffs.append(diff)
    ...
    l_vec = np.mean(left_diffs, axis=0); l_vec /= np.linalg.norm(l_vec)
    f_vec = np.mean(front_diffs, axis=0); f_vec /= np.linalg.norm(f_vec)
...
def check_spatial_relationship_3d(rel_type, pos_s, pos_t, l_vec, f_vec):
    diff = pos_t - pos_s
    if rel_type == "left": return np.dot(l_vec, diff) > 0
    if rel_type == "right": return np.dot(-l_vec, diff) > 0
    if rel_type == "front": return np.dot(f_vec, diff) > 0
    if rel_type == "behind": return np.dot(-f_vec, diff) > 0
    return False
        ...
        for source_idx, targets in enumerate(rel_lists):
            for target_idx in targets:
                if source_idx in anchors_3d and target_idx in anchors_3d:
                    if check_spatial_relationship_3d(rel_type, anchors_3d[source_idx], anchors_3d[target_idx], l_vec, f_vec):
                        score += 1
    return score

# --- EVALUATION LOGIC ---
def print_report(m):
    print("\n" + "="*70 + "\n🏁 FINAL COMPOSITIONAL EVALUATOR (DINO + SigLIP ENSEMBLE)\n" + "="*70)
    print(f"\n[1] DETECTION (Precision: {m['v_matches']/max(1, m['t_pred'])*100:.1f}%, Recall: {m['v_matches']/max(1, m['t_req'])*100:.1f}%)")
    ...
    print(f"    Size (3D Width): {m['c_sz']/v*100:.1f}% | PERFECT: {m['perf']/v*100:.1f}%")
    rel_acc = (m["c_rel"] / max(1, m["t_rel"])) * 100 if m["t_rel"] > 0 else 0.0
    print(f"\n[3] RELATIONAL ACCURACY: {rel_acc:.1f}% ({m['c_rel']}/{m['t_rel']})\n" + "="*70)

def evaluate_generative_clevr(data_dir, split="train", limit=None):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    dataset = CLEVRHybridDataset(root_dir=data_dir, split=split, download=False)
    H, l_vec, f_vec, sz_thresh = calibrate_camera_and_size(dataset.scenes)
    ...
    m = {"t_req": 0, "t_pred": 0, "v_matches": 0, "hallucinations": 0, "c_col": 0, "c_sh": 0, "c_mat": 0, "c_sz": 0, "perf": 0, "t_rel": 0, "c_rel": 0}
    loop_limit = limit if limit is not None else len(dataset.scenes)

    for i, item in enumerate(tqdm(dataset.scenes[:loop_limit])):
        image = Image.open(os.path.join(dataset.image_dir, item["image_filename"])).convert("RGB")
        gt_objects = item.get("objects", [])

        # 1. STAGE 1: DETECTION (Hybrid Prompt + Physics-Based Filter)
        ...
        with torch.no_grad(): outputs = dino_mod(**inputs)
        res = dino_proc.post_process_grounded_object_detection(outputs, inputs.input_ids, box_threshold=0.3, text_threshold=0.25, target_sizes=[image.size[::-1]])[0]

        bboxes = []
        if len(res["boxes"]) > 0:
            keep = torchvision.ops.nms(res["boxes"], res["scores"], iou_threshold=0.65)
            n_boxes = res["boxes"][keep].tolist(); n_scores = res["scores"][keep].tolist()
            sorted_boxes = [b for _, b in sorted(zip(n_scores, n_boxes), reverse=True)]
            for b in sorted_boxes:
                cx1, cy1 = (b[0]+b[2])/2, (b[1]+b[3])/2
                if not any((cx1-(kb[0]+kb[2])/2)**2 + (cy1-(kb[1]+kb[3])/2)**2 < 15**2 for kb in bboxes): bboxes.append(b)

        m["t_req"] += len(gt_objects); m["t_pred"] += len(bboxes)

        # 2. STAGE 2: ENSEMBLE VOTE ATTRIBUTES
        pred_objs = []; all_crops = []
        for b in bboxes:
            x1, y1, x2, y2 = map(int, b)
            full_crop = image.crop((max(0, x1-5), max(0, y1-5), min(image.width, x2+5), min(image.height, y2+5)))
            w, h = x2-x1, y2-y1
            center_crop = image.crop((x1+w//4, y1+h//4, x2-w//4, y2-h//4))
            all_crops.extend([full_crop, center_crop])
        ...
        if all_crops:
            ...
            for idx, b in enumerate(bboxes):
                # Vote by averaging logits from full view and center view
                ...
                pred_objs.append({"bbox": b, "size": "large" if w_3d > sz_thresh else "small", "color": color, "material": "rubber" if "matte" in mat_str else "metal", "shape": shape, "center_3d": get_3d_center(b, H)})

        # 3. HUNGARIAN MATCHING & SPATIAL PERMUTATION
        ...
        for gi, pi in zip(g_idx, p_idx):
            if cost[gi, pi] <= 2:
                m["v_matches"] += 1; po, go = pred_objs[pi], gt_objects[gi]; matched_3d[gi] = po["center_3d"]
                if go["color"] == po["color"]: m["c_col"] += 1
                if go["shape"] == po["shape"]: m["c_sh"] += 1
                if go["material"] == po["material"]: m["c_mat"] += 1
                if go["size"] == po["size"]: m["c_sz"] += 1
                if cost[gi, pi] == 0: m["perf"] += 1
        m["hallucinations"] += len(pred_objs) - len(matched_3d)

        gr = item.get("relationships", {})
        phrase_to_gt = {}
        for gi, ph in enumerate(gt_phrases): phrase_to_gt.setdefault(ph, []).append(gi)
        ambig = [inds for inds in phrase_to_gt.values() if len(inds) > 1 and all(idx in matched_3d for idx in inds)]
        for g_group in ambig:
            best_a = matched_3d.copy(); best_s = score_all_relations(best_a, gr, l_vec, f_vec)
            orig_a = [matched_3d[idx] for idx in g_group]
            for perm in itertools.permutations(orig_a):
                test_a = matched_3d.copy()
                for idx, new_a in zip(g_group, perm): test_a[idx] = new_a
                cur_s = score_all_relations(test_a, gr, l_vec, f_vec)
                if cur_s > best_s: best_s = cur_s; best_a = test_a.copy()
            matched_3d = best_a
        m["c_rel"] += score_all_relations(matched_3d, gr, l_vec, f_vec)
        for r_lists in gr.values():
            for si, targets in enumerate(r_lists):
                for ti in targets:
                    if si in matched_3d and ti in matched_3d: m["t_rel"] += 1

        if (i+1) % 50 == 0: print_report(m)
    print_report(m)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(); parser.add_argument("--data_dir", type=str, required=True); parser.add_argument("--limit", type=int, default=None); args = parser.parse_args()
    evaluate_generative_clevr(args.data_dir, limit=args.limit)