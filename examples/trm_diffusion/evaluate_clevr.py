...
# Import your dataset class for Calibration ONLY
...
# Original CLEVR aspect ratio dimensions
...
def print_report(m):
    print("\n" + "="*70 + "\n🖼️ GENERATIVE MODEL PERFORMANCE REPORT\n" + "="*70)
...
def evaluate_model_samples(samples_dir, clevr_dir, limit=None):
    ...
    for i, item in enumerate(tqdm(samples[:loop_limit])):
        ...
        # 2. STAGE 2: BATCHED ATTRIBUTE CLASSIFICATION
        pred_objs = []; all_crops = []
        for b in bboxes:
        ...
        # 4. SPATIAL PERMUTATION SOLVER (The Identical Twins Fix)
