"""
eval/topodiff_analysis.py — Verbatim vendored copy of the TopoDiff paper's
own metric code, topodiff/topodiff_analysis.py from
https://github.com/francoismaze/topodiff, pinned at commit
9863952acce899c7265e2a344af00a838c4b3680 (2022-12-05, "Adding code and
scripts" — the only commit that has ever touched this file).

Source: "Diffusion Models Beat GANs on Topology Optimization" (Mazé & Ahmed,
AAAI 2023, arXiv:2208.09591). Everything below this docstring is byte-for-
byte identical to the original file — no logic has been changed, fixed, or
reformatted. Only this header comment was added.

License note: the francoismaze/topodiff repository has no LICENSE file
(confirmed via the GitHub API — license: None), so there is no explicit
open-source grant. This is vendored here for internal research/evaluation
use only (reproducing the paper's own FM/VFE/CE/LV metrics against our own
model's samples), not redistributed as part of any package.

Known bug in the vendored code (left unfixed, per "verbatim" above) —
pre_process_summaries (line ~191): the `for j, coord in enumerate(...)`
loop and the `dict_array[i]['load_nodes'] = ...` assignment are NOT
indented inside the outer `for i in range(dict_array.size):` loop (verified
against the raw file bytes, not a formatting artifact). As written, this
means only the LAST entry of `dict_array` ever gets a *freshly computed*
'load_nodes' field; calling it on a dict_array whose entries don't already
carry 'load_nodes' would KeyError on every other entry. In practice this
never bites: every summary .npy shipped with the dataset (both
training_data_summary.npy and test_data_level_{1,2}_summary.npy) already
has 'load_nodes' precomputed and saved from data-generation time, so this
function's recomputation is redundant for every entry it doesn't clobber.

eval/topodiff_eval.py (this package's wrapper, not part of the vendored
code) therefore never calls pre_process_summaries, topodiff_analysis(),
order_list(), or re_order_tab() — those four exist solely to reconstruct
the original authors' own sampler's on-disk output ordering (a
lexicographic-string-sort artifact of their own file-listing code), which
doesn't apply here since we drive our own model and keep our own
puzzle_id-indexed bookkeeping (see datasets/topodiff_dataset.py). Instead,
the wrapper calls the lower-level, bug-free building blocks directly:
create_files, mysolidspy, check_load, compute_vf, check_floating_material —
using 'load_nodes' as already loaded from the summary .npy, and its own
per-sample temp directory (create_files/mysolidspy both already take a
`directory`/`path` argument — the only hardcoded "./fem_files/" path lives
in the unused convenience wrapper fem_compliance_i, which the wrapper
below also doesn't call, precisely to keep this file's own contents
untouched while still being safe to call concurrently / from a training
loop).
"""
