...
def generate_image_batch(
    ...
    # ================= CoT (<think>) planning pass =====================
    # [CoT] Janus two-stage: Janus-Pro cannot emit text and image in one pass,
    ...
    if _think:
        ...
        for _i, _pr in enumerate(prompts):
            ...
            _think_pre.append(VLChatProcessorOutput(
                sft_format=_sft,
                pixel_values=None,
                input_ids=torch.LongTensor(processor.tokenizer.encode(_sft)),
                num_image_tokens=torch.IntTensor([])
            ))
        _tin = processor.batchify(_think_pre)
        _tids = _tin.input_ids.to(device)
        _tmask = _tin.attention_mask.to(device)
        _temb = model.language_model.get_input_embeddings()(_tids)
        if has_input_images and input_image_embeds_list:
            _sid = processor.image_start_id
            for _i in range(batch_size):
                if input_image_embeds_list[_i] is None:
                    continue
                _ix = (_tids[_i] == _sid).nonzero(as_tuple=True)[0]
                if len(_ix) < 1:
                    continue
                _a = _ix[0].item() + 1
                _b = _a + image_token_num_per_image
                if _b <= _temb.shape[1]:
                    _temb[_i, _a:_b, :] = input_image_embeds_list[_i]
        try:
            with torch.no_grad():
                _out = model.language_model.generate(
                    inputs_embeds=_temb,
                    attention_mask=_tmask,
                    max_new_tokens=_max_think,
                    do_sample=False,
            ...
            logger.error("[CoT] planning pass failed (%s) -- falling back to no-CoT", _e)
    # =================== end CoT planning pass =========================
...
def main(args: argparse.Namespace):
    ...
    for batch_idx, (batch_indices, batch_samples, size_key) in enumerate(tqdm(eval_batches, desc='Inference')):
        ...
        for attempt in range(args.num_attempts):
            if all(
                os.path.exists(os.path.join(args.output_dir, f"{filename_prefix}_{sample_ids[i]}_attempt{attempt+1:03d}.png"))
                for i in range(batch_size)
            ):
                continue
            try:
