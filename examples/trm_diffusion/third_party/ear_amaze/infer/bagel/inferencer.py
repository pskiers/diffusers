...
class InterleaveInferencer:
    ...
    @torch.no_grad()
    def _to_model_device(self, generation_input):
        """bagel.py's prepare_* helpers build every tensor on the CPU and leave the
        move to the caller. Only the vae branches ever did it, so text/vit/latent
        inputs reached a CUDA model as CPU tensors ("Expected all tensors to be on
        the same device"). Centralised here so no call site can miss it again."""
    ...
    def update_context_text(self, text, gen_context):
        ...
        generation_input = self._to_model_device(generation_input)
    ...
    def update_context_image_batch(self, images: List[Image.Image], gen_context, vae=True, vit=True):
        ...
        if vit:
            ...
            # prepare_vit_images returns CPU tensors; the vae branch above moves them to
            # the model device but this branch did not, so packed_text_ids reached a CUDA
            # embedding as a CPU tensor -> "Expected all tensors to be on the same device".
            for k, v in generation_input.items():
                if torch.is_tensor(v):
                    generation_input[k] = v.to(model_device)
    ...
    def gen_image(
        ...
        if learn:
            ...
            generation_input = self._to_model_device(generation_input)
            generation_input_cfg_text = self._to_model_device(generation_input_cfg_text)
            generation_input_cfg_img = self._to_model_device(generation_input_cfg_img)
