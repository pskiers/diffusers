...
def _amaze_attempt_filename(metadata, is_circle_dataset, sample_id, attempt_idx):
    """Rebuild the name eval()'s save loop writes a generated image under."""
    maze_config = {}
    if 'metadata' in metadata:
        try:
            if isinstance(metadata['metadata'], str):
                maze_config = json.loads(metadata['metadata']).get('maze_config', {})
            elif isinstance(metadata['metadata'], dict):
                maze_config = metadata['metadata'].get('maze_config', {})
        except (json.JSONDecodeError, TypeError):
            pass
    ...
    if is_circle_dataset:
        size = f"layers_{maze_config.get('layers', 0)}"
    else:
        size = f"{maze_config.get('width', 0)}x{maze_config.get('height', 0)}"
    return f"{size}_{sample_id}_attempt{attempt_idx:03d}.jpg"


def _amaze_load_done_batch(output_dir, metadatas, ids, is_circle_dataset, attempt_idx):
    """Return this batch's already-saved images, or None to generate them.

    A rerun otherwise regenerates everything, so a crash late in a long run throws
    away every image before it. Only a batch that is complete on disk is reused, so
    a partly-written batch is simply redone.
    """
    paths = []
    for metadata, sample_id in zip(metadatas, ids):
        path = os.path.join(
            output_dir, _amaze_attempt_filename(metadata, is_circle_dataset, sample_id, attempt_idx)
        )
    ...
    try:
        ...
        for path in paths:
            ...
            tensors.append(torch.from_numpy(array.transpose(2, 0, 1)).float() / 255.0)
        return torch.stack(tensors, dim=0)
    except Exception as exc:
        print(f"Resume: cannot reuse saved images ({exc}); regenerating this batch.")
        return None
...
def eval(inferencer, inference_hyper, test_dataloader, tokenizer, config, accelerator, eval_reward_fn, executor, autocast, output_dir, num_attempts=1):
    ...
    for idx in indices:
        ...
        # A group is stacked with torch.stack, so every sample in it must share one
        # pixel geometry. The metadata below does not guarantee that: queens rows
        # carry no width/height (every board keys to "0x0") and circle mazes report
        # 10x10 whatever the layer count, so both collapse into a single group of
        # mixed-resolution samples and the stack dies part-way through the run.
        # The input image's own size is what the generated size is derived from,
        # so key on that, and keep the metadata only as a fallback.
        input_image = sample.get("image")
        image_size = getattr(input_image, "size", None) if input_image is not None else None
        if image_size:
            size_key = f"{image_size[0]}x{image_size[1]}"
        elif is_circle_dataset:
            size_key = f"layers_{maze_config.get('layers', 0)}"
        else:
            size_key = f"{maze_config.get('width', 0)}x{maze_config.get('height', 0)}"
        samples_by_size[size_key].append(idx)
    ...
    for attempt_idx in my_attempts:
        ...
        for batch_indices, test_batch, size_key in tqdm(
            ...
            # 使用批量推理：一次性处理整个batch
            # Skip work already on disk (see _amaze_load_done_batch).
            images = _amaze_load_done_batch(
                output_dir, metadatas, ids, is_circle_dataset, attempt_idx
            )
...
def main(_):
    ...
    # CoT: the InterleaveInferencer already supports a <think> planning pass
    # (GEN_THINK_SYSTEM_PROMPT); it was simply never switched on from here.
    _think = bool(int(os.environ.get("THINK", "0")))
    if _think:
        print("[CoT] THINK=1 -- Bagel will emit a <think> plan before each image")
    inference_hyper = dict(
        ...
        think=_think,
        max_think_tokens=int(os.environ.get("MAX_THINK_TOKENS", "1024")),
    )
