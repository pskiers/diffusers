...
class Bagel(PreTrainedModel):
    ...
    def forward_cache_update_vit(
        ...
        # Get the device from vit_model to ensure all tensors are on the same device
    ...
    def forward_cache_update_vae(
        ...
        # 获取 VAE 模型的设备和 dtype，并将输入图像移动到正确的设备和 dtype
    ...
    def generate_image_learn(
        ...
        kl_loss_list=[]
        loss_list=[]
        mse_loss_list=[]  # 用于监控 latent 预测质量
        grad_norm_list=[]  # 用于记录 gradient norm
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=not accelerator.is_local_main_process):
            ...
            # =============== 计算 MSE Loss (仅用于监控) ===============
            # 计算预测的 latent 和实际采样的 latent 之间的 MSE
            with torch.no_grad():
                mse_loss = torch.nn.functional.mse_loss(prev_sample_mean, prev_latents[i])
            ...
            # grpo logic
            ratio = torch.exp(log_prob - sample["log_probs"][i]).float()
            #print('ratio', ratio)
            #print(f"old: {log_prob} new: {sample['log_probs'][i]}")
            #print('ratio', ratio)
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                print(f"[Warning] Loss is NaN/Inf at step {i}, skipping backward. Policy: {policy_loss.item()}, Ratio Max: {ratio.max().item()}")
        ...
        # 计算 MSE Loss 的平均值 (仅用于监控)
        if len(mse_loss_list) > 0:
            mse_loss = torch.stack(mse_loss_list).mean().detach()
        else:
            mse_loss = torch.tensor(0.0, device=loss.device)
        
        # 计算 gradient norm 的平均值
        if len(grad_norm_list) > 0:
            grad_norm = torch.tensor(grad_norm_list, device=loss.device).mean().detach()
        else:
            grad_norm = torch.tensor(0.0, device=loss.device)
            
        return clipfrac, clipfrac_gt_one, clipfrac_lt_one, policy_loss, kl_loss, loss, mse_loss, grad_norm
    ...
    def _sde_step_with_logprob(
        ...
        # 【防护 1】输入清洗
         ...
         #    model_output = torch.nan_to_num(model_output)

        if sde_type == "sde":
            #print(f"noise_level: {noise_level} {torch.sqrt(timestep / (1 - torch.where(timestep == 1, sigma_max, timestep)))*noise_level}")
    ...
    def _forward_flow(
        ...
        # =============== 临时切换到eval模式以使用forward_inference ===============
        # 因为我们只训练connector，LLM本身是冻结的，使用inference模式避免参数不匹配
        ...
        #         packed_sequence=packed_sequence, # packed_sequence.shape=[1026, 3584]
        #         sample_lens=packed_seqlens,   # packed_seqlens=tensor([1026])
