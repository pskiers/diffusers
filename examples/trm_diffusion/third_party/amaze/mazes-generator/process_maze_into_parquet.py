...
def create_parquet_dataset(maze_dir, no_markers_dir, solution_dir, metadata_dir, output_path,
    ...
    # Use the "_test" suffix so the output matches AmazeDataset / the amaze
    # Hydra config (split="test") and the queen converter's naming scheme.
    val_path = os.path.join(out_dir, f"{stem}_test.parquet")
...
def main():
    ...
    parser.add_argument('--output', default='./maze-dataset/maze_dataset.parquet',
                        help='输出Parquet基名 (将生成 *_train.parquet 和 *_test.parquet)')
    ...
    print(f"输出基名: {args.output}  →  *_train.parquet 和 *_test.parquet")
