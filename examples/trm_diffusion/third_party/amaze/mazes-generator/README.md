...
Use a modern Node.js version with ES module support. Node 18+ works.

The generator imports `jsdom` and `sharp`. The versions are pinned in
`package.json` to `jsdom@^24` and `sharp@^0.33.5`, which are the last releases
that support Node 18. **Do not upgrade to `jsdom@27` / `sharp@0.35` unless you
are on Node 20+** — those newer versions fail on Node 18 with
`ERR_REQUIRE_ESM` (jsdom) and `Could not load the "sharp" module` (sharp).

With the checked-in `package.json`, just run:

```bash
npm install
```

If you are on Node 20+ and want the latest dependencies instead:

```bash
npm install jsdom@latest sharp@latest
```
...
all matched samples in the train split. The converter writes
`<output_stem>_train.parquet` and `<output_stem>_test.parquet`; the `_test`
file is what `AmazeDataset` loads for the `split="test"` validation set.
