...
Vendored from google-research/ibc (Apache 2.0 license) —
environments/block_pushing/block_pushing.py. Changes:
  - import paths adapted to this package
  - the `metrics` import (and thus a hard `tf_agents`/TensorFlow dependency)
    is dropped — it was only used inside get_metrics(), which our own
    dp_eval_callbacks.py never calls (we compute our own score independently)
  - *_URDF_PATH constants point at real filesystem paths in this package's
    assets/ dir instead of Google-internal path conventions
"""
...
from third_party.block_pushing import utils_pybullet
from third_party.block_pushing import xarm_sim_robot
from third_party.block_pushing.pose3d import Pose3d
from third_party.block_pushing.utils_pybullet import ObjState
from third_party.block_pushing.utils_pybullet import XarmState
import numpy as np
...
BLOCK_URDF_PATH = os.path.join(_ASSETS_DIR, 'block.urdf')
PLANE_URDF_PATH = ('third_party/bullet/examples/pybullet/gym/pybullet_data/'
                   'plane.urdf')
WORKSPACE_URDF_PATH = os.path.join(_ASSETS_DIR, 'workspace.urdf')
ZONE_URDF_PATH = os.path.join(_ASSETS_DIR, 'zone.urdf')
INSERT_URDF_PATH = os.path.join(_ASSETS_DIR, 'insert.urdf')
...
WORKSPACE_URDF_PATH_REAL = os.path.join(_ASSETS_DIR, 'workspace_real.urdf')
...
class BlockPush(gym.Env):
  ...
  def get_metrics(self, num_episodes):
    # Upstream's metrics.py needs tf_agents (-> TensorFlow), which we don't
    # want as a hard dependency just for this unused helper. dp_eval_callbacks.py
    # computes its own score independently and never calls this method.
    from third_party.block_pushing import metrics as block_pushing_metrics
    metrics = [
...
#
# This whole gym-registry dance is not needed for our own usage (we
# instantiate BlockPush/BlockPushMultimodal directly as classes, never via
# gym.make(...)), and gym 0.26's registry is a plain dict (no .env_specs
# attribute) rather than the object this code was written against — so the
# de-registration/registration below is wrapped defensively rather than
# adapted exactly, since we don't rely on its outcome.
try:
