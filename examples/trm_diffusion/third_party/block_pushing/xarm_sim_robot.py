...
environments/utils/xarm_sim_robot.py. Only changes: import paths adapted to
this package, and the *_URDF_PATH constants below point at real filesystem
paths instead of Google-internal ("third_party/py/...", "robotics/...")
path conventions that don't resolve outside that environment.

XARM_URDF_PATH still resolves via pybullet_data.getDataPath() (see
utils_pybullet.load_urdf) — the xarm6_robot.urdf asset isn't published in
google-research/ibc or real-stanford/diffusion_policy's public repos, but
IS bundled in the pybullet-svl package (Stanford Vision & Learning Lab's
pybullet fork, which upstream diffusion_policy's own conda_environment.yaml
pins) at pybullet_data/xarm/xarm6_robot.urdf. Install pybullet-svl instead
of plain pybullet for this to resolve.
"""
import os
...
XARM_WHITE_URDF_PATH = XARM_URDF_PATH  # xarm6_robot_white.urdf isn't published anywhere; fall back to the default color.
SUCTION_URDF_PATH = os.path.join(_ASSETS_DIR, 'suction', 'suction-head-long.urdf')
CYLINDER_URDF_PATH = os.path.join(_ASSETS_DIR, 'suction', 'cylinder.urdf')
CYLINDER_REAL_URDF_PATH = os.path.join(_ASSETS_DIR, 'suction', 'cylinder_real.urdf')
